...
export default function injectDependencies( Animated, PanResponder ){
	...
	return class InteractableView extends Component {
		static propTypes = {
			...
			freewillAfter: PropTypes.object,
		...
		endDrag(){
			...
			this.addTempSnapToPointBehavior(snapPoint, projectedCenter);
		...
		addTempSnapToPointBehavior( snapPoint, projectedCenter ) {
			if (!snapPoint)
			  return;

			let { snapPoints, freewillAfter, onSnap, onSnapStart } = this.props

			if (freewillAfter !== undefined && typeof freewillAfter.x === 'number' && projectedCenter && freewillAfter.x >= projectedCenter.x)
				return;
				
			if (freewillAfter !== undefined && typeof freewillAfter.y === 'number' && projectedCenter && freewillAfter.y >= projectedCenter.y)
				return;
